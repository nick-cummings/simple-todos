...
The `/mockups/*` routes (`labels`, `label-picker`, `auto-label`,
`reminders`, `undo-toast`, `recurring`) host unfinished design mockups
backed by large mock components under `src/components/mockups/`. They
render in local dev
