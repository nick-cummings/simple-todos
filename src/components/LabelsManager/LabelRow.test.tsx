...
function renderRow(
  ...
  const root = utils.container.querySelector<HTMLElement>("[data-label-name]");
...
describe("<LabelRow> swipe gestures", () => {
  it("rejects vertical drags (lets the scroll container own them)", () => {
    ...
    fireEvent.pointerUp(draggable, { clientX: 102, clientY: 20, pointerId: 1 });
  ...
  it("ignores movement under the 6px capture threshold", () => {
    ...
    fireEvent.pointerUp(draggable, { clientX: 97, clientY: 100, pointerId: 1 });
    expect(transformX(draggable)).toBe(0);
  });
  ...
  it("triggers onDelete (after exit animation) when drag passes the delete threshold", () => {
    ...
    fireEvent.pointerUp(draggable, { clientX: 30, clientY: 50, pointerId: 1 });
