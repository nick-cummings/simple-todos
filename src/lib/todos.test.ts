...
describe("dedupeLabels", () => {
    ...
    it("dedupes case-insensitively but preserves first occurrence casing", () => {
        expect(dedupeLabels(["Foo", "foo", "FOO", "bar"])).toEqual([
            "Foo",
            "bar",
        ]);
    });
...
describe("sortTodos", () => {
    ...
    it("completed orders open before done; ties broken by createdDesc", () => {
        ...
        expect(
            sortTodos([t2, t1, t3], "completed").map((t) => t.title),
        ).toEqual(["open-new", "open-old", "done-new"]);
    });
    it("dueDate sorts by ISO string ascending; missing due dates go to end", () => {
        ...
        expect(
            sortTodos([noDue1, due2, noDue2, due1], "dueDate").map(
                (t) => t.title,
            ),
...
describe("filterTodos", () => {
    ...
    describe("status filter", () => {
        ...
        it("open-only hides completed todos", () => {
            expect(filterTodos([open1, done1], [], "", set("open"))).toEqual([
                open1,
            ]);
        });
        it("done-only shows only completed todos", () => {
            expect(filterTodos([open1, done1], [], "", set("done"))).toEqual([
                done1,
            ]);
        });
    });
    ...
    describe("query filter", () => {
        ...
        it("matches description", () => {
            expect(filterTodos([open1, open2], [], "highlight")).toEqual([
                open2,
            ]);
        });
        it("trims surrounding whitespace in the query", () => {
            expect(filterTodos([open1, open2], [], "  milk  ")).toEqual([
                open1,
            ]);
        });
    ...
    describe("label filter", () => {
        it("OR-matches a single label", () => {
            expect(filterTodos([open1, open2], ["errands"], "")).toEqual([
                open1,
            ]);
        });
...
describe("recurrenceLabel", () => {
    ...
    it("formats every-N forms with plural units", () => {
        ...
        expect(recurrenceLabel({ every: 3, unit: "week" })).toBe(
            "Every 3 weeks",
        );
        expect(recurrenceLabel({ every: 6, unit: "month" })).toBe(
            "Every 6 months",
        );
    });
});
