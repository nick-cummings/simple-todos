...
> {
  ...
  override render(): ReactNode {
    if (this.state.error) {
      return <Error error={this.state.error} unstable_retry={() => null} />;
    }
...
describe("<Error> — page-level boundary", () => {
  ...
  it("invokes unstable_retry when Try again is clicked", async () => {
    ...
    render(<Error error={new globalThis.Error("x")} unstable_retry={retry} />);
  ...
  it("shows the error digest when present and omits it otherwise", () => {
    ...
    rerender(
      <Error error={new globalThis.Error("y")} unstable_retry={vi.fn()} />,
    );
  ...
  it("calls location.reload when Reload page is clicked", async () => {
    ...
    const user = userEvent.setup();
    render(
      <Error error={new globalThis.Error("x")} unstable_retry={vi.fn()} />,
    );
